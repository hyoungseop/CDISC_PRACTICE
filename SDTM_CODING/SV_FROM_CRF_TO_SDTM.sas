...
*5. VISITDY 계산;
*기준일 이상이면 +1, 이전이면 +0;
VISITDY = intck('day',SVSTDAT,RFSTDAT,'c') + (SVSTDAT>=RFSTDAT);
...
*기준일 이상이면 +1, 이전이면 +0;
*SVSTDAT - RFSTDAT;
SVSTDY = intck('day',SVSTDAT,RFSTDAT,'c') + (SVSTDAT>=RFSTDAT);

*8. SVENDY 계산;
*기준일 이상이면 +1, 이전이면 +0;
*SVENDAT - RFSTDAY;
SVENDY = intck('day',SVENDAT,RFSTDAT,'c') + (SVENDAT>=RFSTDAT);
run;
